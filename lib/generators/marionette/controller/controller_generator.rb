  ...
  module Generators
    class ControllerGenerator < Rails::Generators::NamedBase
      ...
      def create_controller
        subbed_actions.each do |action|
          %W{ controller view }.each do |type|
            ...
            template "#{type}.js.coffee", "#{backbone_path}/apps/#{module_name_underscore}/#{action}/#{action}_#{type}.coffee"
          end
        end
      end
      ...
      def create_template
        subbed_actions.each do |action|
          @controller = action.capitalize
          template "layout.hamlc", "#{backbone_path}/apps/#{module_name_underscore}/#{action}/templates/#{action}_layout.hamlc"
        end
      end
