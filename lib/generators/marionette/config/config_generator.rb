  ...
  module Generators
    class ConfigGenerator < Rails::Generators::Base
      ...
      def create_marionette_renderer
        template 'renderer.js.coffee', "#{config_path}/marionette/renderer.coffee" if yes?("Override Marionette Renderer? (y/n)")
      end

      def create_base_entities
        if yes?("Create base entities for Models and Collections? (y/n)")
          template "models.js.coffee", "#{backbone_path}/entities/_base/models.coffee"
          template "collections.js.coffee", "#{backbone_path}/entities/_base/collections.coffee"
        end
      end

      def create_base_controller
        if yes?("Create base controller for Marionette Controllers? (y/n)")
          template "controllers.js.coffee", "#{backbone_path}/controllers/_base_controller.coffee"
        end
      end
      ...
      def create_base_views
        @base_views = if yes?("Create base views for Marionette View, ItemView, Layout, CollectionView, CompositeView? (y/n)")
          %W{_view itemview layout collectionview compositeview}.each do |view|
            template "views/#{view}.js.coffee", "#{backbone_path}/views/_base/#{view}.coffee"
          end
        end
      end
